...
class SplineCanvas(QWidget):
    ...
    def add_to_value_labels(self, nlabel):
        self.value_labels.append(nlabel)
    ...
    def update_value_label(self):
        ...
        # Update each label in the value_labels array
        for i, label in enumerate(self.value_labels):
            label.setText(f"Value {i}: {current_value:.2f}")
    ...
    def mouseMoveEvent(self, event):
        ...
        if self.selected_point is not None:
            old_t, old_v, h_out, h_in = self.points[self.selected_point]
            ...
            # Move handles by same delta (if handles exist)
            if h_out:
                h_out = (h_out[0] + dt, h_out[1] + dv)
            if h_in:
                h_in = (h_in[0] + dt, h_in[1] + dv)
